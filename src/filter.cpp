...
std::vector<std::vector<int>> createDiskKernel(int radius)
{
    ...
    std::vector<std::vector<int>> kernel(diameter,
                                         std::vector<int>(diameter, 0));
    int center = radius;
...
void erode(ImageView<rgb8> in, const std::vector<std::vector<int>>& kernel,
           int radius)
{
    Image<rgb8> copy = clone(
        in); // Faire une copie temporaire de l'image pour éviter la corruption
    int diameter = 2 * radius + 1;
            ...
            for (int ky = 0; ky < diameter; ++ky)
            {
                for (int kx = 0; kx < diameter; ++kx)
                {
                    if (kernel[ky][kx] == 1)
                    {
                        ...
                        // std::cout << "val: " << (int)kernel_pixel[nx].r <<
                        // std::endl;
                    }
                }
            }
...
void dilate(ImageView<rgb8> in, const std::vector<std::vector<int>>& kernel,
            int radius)
{
    int diameter = 2 * radius + 1;
            ...
            for (int ky = 0; ky < diameter; ++ky)
            {
                for (int kx = 0; kx < diameter; ++kx)
                {
                    if (kernel[ky][kx] == 1)
                    {
                        ...
                        // std::cout << "val: " << (int)kernel_pixel.r <<
                        // std::endl; std::cout << "max_pixel: " << max_pixel <<
                        // std::endl;
                    }
                }
            }
                             ...
                             std::vector<rgb8> initialPixels)
{
    for (int y = 0; y < in.height; y++)
    {
        for (int x = 0; x < in.width; x++)
        {
            ...
            {
                in.buffer[index].r = std::min(
                    255, static_cast<int>(initialPixels[index].r + 0.5 * 255));
